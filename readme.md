...
Today, we are going to use TypeScript, Express.js, and TypeORM to create an enterprise level Rest API with JWT authentication and role based authorization. The objective is to create a repository that you can use as a base for your real life projects.
...
We wrote a lot of code and it is ok to lose track of in which order each file is called. For that reason I created a simple chart that exemplifies the flow of a user's request that requires a role check and uses a function from userController.
...
Node.js itself cannot run .ts files natively. For that reason it is important to know the following tools:
"tsc" - creates a /build folder and converts all your .ts to .js files.
"ts-node" - allows node to run .ts projects. Not recommended for production uses.
"ts-node-dev" - same as above, but allows you to restart the node server every time you change a file.

To better setup development and production environments, we will modify the script section of the package.json. 

<iframe src="https://medium.com/media/5c66379601b23108065e13e7a21c0414" frameborder=0></iframe>

Finally, we add a last line called migration:run. Some windows users get an error trying to run TypeORM migrations from npm. Running it directly from the node modules folder solves the problem.
...
Then, we are going to modify the generated file:
...
Finally, your server is ready to go. Just use Postman, or any other tool, and make some requests.

## Maintainer

This project is maintained by Akshay Basavalingaiah.

I am a Backend Software Engineer with over 5 years of experience building scalable APIs, microservices, and distributed systems. My expertise includes Golang, Java, Python, and cloud infrastructure with AWS and Docker. I focus on improving system performance, reliability, and observability in production environments.

**Contact Information:**
- Email: akshayb220194@gmail.com
- GitHub: https://github.com/akshayb220194
- LinkedIn: https://www.linkedin.com/in/akshay-basavalingaiah/

Original architecture inspired by the work of andregardi. All technical documentation and core logic have been preserved and are actively maintained for current standards.